...
reelLabRoutes.post("/reel-lab/storyboard", async (c) => {
  try {
    ...
    const pageName = normalizeBrand(String(body.pageName || "Zawaago")); const topic = String(body.topic || "AI Agent vs Chatbot — what is the real difference?").trim(); const language = String(body.language || "Hinglish").trim(); const audience = String(body.audience || "Business Owners and Professionals").trim();
...
reelLabRoutes.post("/reel-lab/assets", async (c) => {
  try {
    ...
    const form = await c.req.formData(); const pageName = normalizeBrand(String(form.get("pageName") || "Zawaago")); const file = form.get("file");
