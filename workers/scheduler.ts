...
async function validateScheduledImage(env: Env, imageUrl: string, request?: Request) {
  ...
  let objectKey = "";
  try { objectKey = decodeURIComponent(encodedKey); } catch { throw new Error("Scheduled image URL is invalid."); }
  ...
  if (!object) throw new Error("Scheduled image is no longer available in R2. Generate the visual again.");
  const contentType = object.httpMetadata?.contentType || "";
  if (!contentType.toLowerCase().startsWith("image/")) throw new Error("Scheduled R2 asset is not an image.");
  if (object.size <= 1000) throw new Error("Scheduled image asset is unexpectedly small. Regenerate the visual.");
}
