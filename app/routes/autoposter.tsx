...
export default function Autoposter() {
  ...
  async function scheduleSeries() {
    ...
    setBusy("series");
    setStatus({ type: "info", text: `Preparing ${series.length} branded visuals and schedules…` });
    try {
      const posts: Array<{ caption: string; imageUrl: string; withImage: boolean }> = [];
      for (let index = 0; index < series.length; index += 1) {
        const post = series[index];
        setStatus({ type: "info", text: `Generating branded visual ${index + 1}/${series.length}…` });
      ...
      setStatus({ type: "info", text: `Queueing ${posts.length} episodes on ${selectedPage.name}…` });
      ...
      setShowSeries(false);
      setStatus({ type: "success", text: `${scheduleData.schedules?.length || posts.length} posts scheduled successfully on ${selectedPage.name}. Each visual uses the real brand logo.` });
    } catch (error) {
