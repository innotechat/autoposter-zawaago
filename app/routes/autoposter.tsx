...
export default function Autoposter() {
  ...
  async function generateImage() {
    ...
    try {
      ...
      setImageUrl(data.imageUrl || ""); setImagePrompt(data.prompt || imagePrompt); setActiveTab("image"); setStatus({ type: "success", text: `Visual ready · ${data.source === "cloudflare-flux-r2" ? "Cloudflare AI + R2" : "high-resolution fallback"}.` });
    } catch (error) { setStatus({ type: "error", text: friendlyError(error) }); } finally { setBusy(null); }
  }
  ...
  async function copyCaption() {
    if (!caption) return;
    try { await navigator.clipboard.writeText(caption); setCopied(true); setStatus({ type: "success", text: "Caption copied to clipboard." }); window.setTimeout(() => setCopied(false), 1800); } catch { setStatus({ type: "error", text: "Clipboard access was blocked by the browser." }); }
  }
