...
export default function Autoposter() {
  ...
  return (
    <main className="autoposter-shell min-h-screen text-[#171717]">
      ...
      {showHealth && <div className="modal-backdrop" onClick={() => setShowHealth(false)}><div className="modal-card" onClick={(e) => e.stopPropagation()}><div className="modal-heading"><h3>System health</h3><button className="icon-button small" onClick={() => setShowHealth(false)}><X size={18} /></button></div><pre>{JSON.stringify(health, null, 2)}</pre></div></div>}
      {showSettings && <div className="modal-backdrop" onClick={() => setShowSettings(false)}><div className="modal-card" onClick={(e) => e.stopPropagation()}><div className="modal-heading"><h3>Studio settings</h3><button className="icon-button small" onClick={() => setShowSettings(false)}><X size={18} /></button></div><p>Advanced creative controls are available in the brief. Persistent database settings, asset management and scheduling are part of the next production phases.</p></div></div>}
    </main>
  );
}
