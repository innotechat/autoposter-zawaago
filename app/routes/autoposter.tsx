...
const seriesIntervals = [1, 3, 6, 12, 24, 48, 72, 168];
...
export default function Autoposter({ fetcher, children }: { fetcher?: FetchLike; children?: React.ReactNode } = {}) {
  ...
  const [seriesStartAt, setSeriesStartAt] = useState(() => toDateTimeLocalValue(new Date(Date.now() + 60 * 60 * 1000)));
  ...
  async function generateSeries() {
    ...
    try {
      ...
      setSeriesStartAt(toDateTimeLocalValue(new Date(Date.now() + 60 * 60 * 1000)));
  ...
  async function scheduleSeries() {
    ...
    if (!seriesStartAt) return setStatus({ type: "error", text: "Choose a start date and time for the series." });
    const start = new Date(seriesStartAt);
    if (Number.isNaN(start.getTime()) || start.getTime() <= Date.now() + 30_000) return setStatus({ type: "error", text: "Choose a valid future start date/time." });
