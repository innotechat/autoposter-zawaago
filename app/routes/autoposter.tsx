...
import {
  ArrowClockwise, CheckCircle, ClipboardText, ClockCounterClockwise, CalendarCheck, CloudArrowUp, Copy, FacebookLogo, GearSix, ImageSquare, MagicWand, PencilSimple, Pulse, Sparkle, SpinnerGap, Trash, WarningCircle, X,
...
function buildPromptPresets(page: string, contentType: string, tone: string, language: string, audience: string, visualStyle: string, aspectRatio: string, branding: string, logoPosition: string, cta: string) {
  ...
  const subject = educational ? "clear visual explanation of the core concept, with one strong real-world example and an intuitive visual metaphor" : contentType === "Product Promotion" ? "premium product/service hero composition showing the practical outcome and value for the target audience" : contentType === "Case Study" ? "credible before-and-after business scenario showing the problem, intervention and practical result without invented metrics" : contentType === "AI News & Insights" ? "current-looking technology editorial visual focused on the idea, avoiding fake headlines, fake logos and fabricated UI" : "strong editorial concept that communicates one clear business or technology idea at a glance";
  const cleanBrand = branding === "No branding" ? "Do not include any logo, brand name or watermark in the artwork." : `Keep a clean ${logoPosition.toLowerCase()} safe area for the ${brand} logo to be overlaid later; never draw or invent the logo itself.`;
...
export default function Autoposter() {
  ...
  function openScheduler() {
    if (caption.trim()) localStorage.setItem("autoposter:schedule-draft", JSON.stringify({ page, caption, imageUrl }));
    window.location.href = "/schedule";
  }

  const selectedPage = pages.find((item) => item.id === page) ?? pages[0];
  const canPublish = caption.trim().length > 0 && !busy;
  const progress = busy === "generate" ? ["Understanding brief", "Writing caption", "Generating high-quality visual"] : busy === "image" ? ["Preparing creative brief", "Generating high-quality visual", "Preparing preview"] : busy === "series" ? ["Planning the learning journey", "Writing connected lessons", "Generating branded episode visuals"] : [];
  ...
  async function generate() {
    ...
    try {
      ...
      setStatus({ type: "success", text: "Draft ready — caption and branded visual generated automatically." });
  ...
  async function generateSeries() {
    ...
    try {
      ...
      setSeries(completed); setShowSeries(true); setActiveTab("caption");
      setStatus({ type: "success", text: `${completed.length}-part education series is ready with all visuals generated and branded.` });
    } catch (error) { setStatus({ type: "error", text: friendlyError(error) }); } finally { setBusy(null); }
  }

  async function scheduleSeries() {
    if (!series.length) return setStatus({ type: "error", text: "Generate the education series first." });
    if (series.some((post) => !post.imageUrl)) return setStatus({ type: "error", text: "Every episode must have a generated visual before scheduling." });
    const raw = window.prompt("Enter the series start date/time (for example: 2026-09-13 10:00)");
    if (!raw) return;
    const start = new Date(raw);
    if (Number.isNaN(start.getTime()) || start.getTime() <= Date.now() + 30_000) return setStatus({ type: "error", text: "Enter a valid future start date/time." });
  ...
  function useSeriesPost(post: SeriesPost) {
    setCaption(post.caption); setImagePrompt(post.visualPrompt); setImageUrl(post.imageUrl || ""); setActiveTab(post.imageUrl ? "image" : "caption"); setShowSeries(false); setTopic(`${topic} · ${post.title}`); setStatus({ type: "info", text: `Episode ${post.episode} loaded with its generated visual.` });
  }
  ...
  return (
    ...
    {showSeries && <div className="modal-backdrop" onClick={() => setShowSeries(false)}><div className="modal-card max-w-3xl" onClick={(e) => e.stopPropagation()}><div className="modal-heading"><div><span className="modal-eyebrow"><Sparkle size={14} weight="fill" /> EDUCATION SERIES</span><h3>{series.length}-part learning series</h3><p>Generated from your current Page, topic, tone, language, audience and creative settings.</p></div><button className="icon-button small" onClick={() => setShowSeries(false)} aria-label="Close series"><X size={18} /></button></div><div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><div className="rounded-xl border border-black/8 bg-[#faf9f7] p-3"><small className="block text-xs text-black/45">Posts</small><strong>{series.length}</strong></div><div className="rounded-xl border border-black/8 bg-[#faf9f7] p-3"><small className="block text-xs text-black/45">Interval</small><strong>{intervalHours}h</strong></div><div className="rounded-xl border border-black/8 bg-[#faf9f7] p-3"><small className="block text-xs text-black/45">Page</small><strong>{page}</strong></div><div className="rounded-xl border border-black/8 bg-[#faf9f7] p-3"><small className="block text-xs text-black/45">Language</small><strong>{language}</strong></div></div><div className="space-y-3">{series.map((post) => <article key={post.episode} className="rounded-2xl border border-black/8 bg-white p-4"><div className="flex items-start gap-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-black text-xs font-bold text-white">{post.episode}</span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><h4 className="font-semibold">{post.title}</h4><p className="mt-1 text-sm font-medium text-black/65">{post.hook}</p></div>{post.imageUrl && <img src={post.imageUrl} alt={`Episode ${post.episode} visual`} className="h-16 w-16 shrink-0 rounded-lg object-cover" />}</div><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-black/75">{post.caption}</p><details className="mt-3"><summary className="cursor-pointer text-xs font-semibold text-black/55">Visual direction</summary><p className="mt-2 rounded-lg bg-[#faf9f7] p-3 text-xs leading-5 text-black/65">{post.visualPrompt}</p></details><div className="mt-3 flex flex-wrap gap-2"><button className="secondary-action compact" onClick={() => useSeriesPost(post)}><PencilSimple size={15} /> Use episode</button><button className="secondary-action compact" onClick={() => copyText(post.caption, post.episode)}>{seriesCopied === post.episode ? <CheckCircle size={15} weight="fill" /> : <Copy size={15} />} {seriesCopied === post.episode ? "Copied" : "Copy"}</button></div></div></div></article>)}</div><div className="mt-4 rounded-xl border border-black/8 bg-[#faf9f7] p-3 text-xs leading-5 text-black/55">All episode visuals are generated before this series opens. Scheduling uses those exact generated branded assets; it does not regenerate them.</div><div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button className="secondary-action compact" onClick={() => setShowSeries(false)} disabled={!!busy}>Review later</button><button className="primary-action compact" onClick={scheduleSeries} disabled={!!busy || !series.length}><CalendarCheck size={15} /> {busy === "series" ? "Scheduling…" : "Schedule full series"}</button></div></div></div>}
